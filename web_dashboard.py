...
def get_report(report_type):
    # 1. Custom Heatmap Handler (Direct from Cache)
    if report_type == "Whale Movement":
        data = get_data_internal() # Assuming get_data_internal is defined elsewhere and accessible
        return jsonify({"content": get_whale_heatmap_report(data)})

    # 2. Check Cache for heavy reports
    try:
